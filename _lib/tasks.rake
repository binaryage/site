...
namespace :shared do
  ...
  task :status do
    ...
    puts "#{'⚠️  WARNING:'.yellow} Automatic sync via hooks is unreliable due to git's"
    puts "   submodule architecture. Use #{'rake shared:sync'.blue} for manual sync instead."
    puts

    enabled = File.exist?(SYNC_ENABLED_FLAG)
    puts "Auto-sync hooks: #{enabled ? 'enabled'.green : 'disabled'.red} (not recommended)"
    puts

    if !enabled
      puts "For manual sync: #{'rake shared:sync'.blue}"
      puts
    end
  ...
  desc 'Enable automatic shared submodule synchronization (NOT RECOMMENDED)'
  task :enable do
    puts "#{'⚠️  WARNING:'.yellow} Automatic sync via post-commit hooks is unreliable."
    puts "Git's submodule architecture reverts changes made during commit hooks."
    puts
    puts "Recommended: Use #{'rake shared:sync'.blue} for manual synchronization instead."
    puts
    print "Continue with hook installation anyway? (y/N) "
